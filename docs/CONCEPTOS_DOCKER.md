...
| `EXPOSE 8002` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    subgraph PC["Su PC — Docker Desktop (el 'centro de datos')"]
        subgraph RED["red interna del compose (LAN virtual, con DNS propio)"]
            APIFACTURAS["SERVIDOR DE APLICACIONES<br/>contenedor api-facturas<br/>hostname: api-facturas · escucha en 8002"]
            POSTGRES[("SERVIDOR DE BASE DE DATOS<br/>PostgreSQL · contenedor postgres<br/>hostname: postgres · escucha en 5432")]
        end
    end
    NAV -->|"localhost:8002"| APIFACTURAS
    APIFACTURAS -->|"postgres:5432 (DNS de Docker)"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15432"| POSTGRES
```
...
nombre de la carpeta: `proyecto_paradigmas1-...`). Si el `docker-compose.yml`
...
```
Found orphan containers ([proyecto_paradigmas1-front-1 ...]) for this project.
```
...
docker network create proyecto_paradigmas1_default

# 2. postgres
docker run -d --name postgres --network proyecto_paradigmas1_default --restart unless-stopped `
...
docker run -d --name api-facturas --network proyecto_paradigmas1_default --restart unless-stopped `
  -e "DB_POSTGRES=postgresql+asyncpg://paradigmas:paradigmas123@postgres:5432/bdfacturas_postgres_local" `
  -v "${PWD}/api_facturas:/app" `
  -p 8002:8002 api-facturas uvicorn main:app --host 0.0.0.0 --port 8002 --reload
...
```yaml
URL_API: http://api-facturas:8002
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8002` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8002` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8002:8002`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8002`, el front
...
```powershell
docker network inspect proyecto_paradigmas1_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_paradigmas1` produce
la red `proyecto_paradigmas1_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_paradigmas1-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_paradigmas1_default has active endpoints (name:"proyecto_paradigmas1-api-facturas-1" …)
```
